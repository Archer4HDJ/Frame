    ...
    public static class JsonUtils
    {
        ...
        private static object ClassOrStructToJsonObject(object data)
        {
            ...
            for (int i = 0; i < fields.Length; i++)
            {
                ...
                bool isNotJsonSerialized = false;
                object[] attrs = f.GetCustomAttributes(true);
                for (int j = 0; j < attrs.Length; j++)
                {
                    NotJsonSerializedAttribute attribute = (NotJsonSerializedAttribute)attrs[j];
                    if (attribute != null)
                    {
                        isNotJsonSerialized = true;
                        break;
                    }
                }
                if (isNotJsonSerialized)
                    continue;
                if (f.IsStatic || f.IsLiteral)//常数
                    continue;
                try
                {
        ...
        public static bool IsSupportBaseValueParseJson(Type t)
        {
        ...
        private static object JsonObjectToClassOrStruct(object jsonObj, Type type)
        {
            ...
            for (int i = 0; i < nameList.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        value = ChangeJsonDataByType(property.PropertyType, value);
