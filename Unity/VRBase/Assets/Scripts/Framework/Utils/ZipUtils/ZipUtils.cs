...
public class ZipUtils
{
    ...
    public static void ZipDirectory(string dirToZip, string zipedFileName, int compressionLevel = 9)
    {
        ...
        using (var zipoutputstream = new ZipOutputStream(File.Create(zipedFileName)))
        {
            ...
            Crc32 crc = new Crc32();
            string[] allFilePath = PathUtils.GetDirectoryFileNames(dirToZip, null, true);
            for (int i =0;i< allFilePath.Length;i++)
            {
                string tempPath = allFilePath[i];
                FileStream fs = new FileStream(tempPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
                byte[] buffer = new byte[fs.Length];
                fs.Read(buffer, 0, buffer.Length);
                // ZipEntry entry = new ZipEntry(item.Key.ToString().Substring(dirToZip.Length + 1));
                ZipEntry entry = new ZipEntry(Path.GetFileName(tempPath))
                {
